...
public:
	...
	void init()
	{
		ifstream fin("/assets/scores.dat", ios::binary);
		if(fin.is_open())
		{
	...
	void save()
	{
		ofstream fout("/assets/scores.dat", ios::binary);
...
void load(int scr)
{
	...
	pstart.init();
	game.load();
	font.load(&mxhwnd, getPath("SmallNeon.png"));
	paint.init(&mxhwnd);
...
int main(int argc, char *argv[])
{
	if(mxhwnd.init("MpSDL", 640,480,32, false) != -1)
	{
		...
		emscripten_set_main_loop(mxHwnd::LoopEvent, 0, 1);
	}
	return (0);
}
